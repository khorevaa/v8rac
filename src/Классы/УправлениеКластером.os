...
// Возвращает значение абсолютного пути к клиенту администрирования 1С
//
// Возвращаемое значение:
//   Строка - Путь к клиенту администрирования
//
Функция ПолучитьПутьКлиентаАдминистрирования() Экспорт

	Возврат ?(ЗначениеЗаполнено(ПутьКлиентаАдминистрирования), ПутьКлиентаАдминистрирования, "");

КонецФункции

// Возвращает адрес сервера 1С
//
// Возвращаемое значение:
//   Строка - Текущий адрес сервера 1С
//
Функция ПолучитьАдресСервера() Экспорт

    Возврат ?(ЗначениеЗаполнено(АдресСервера), АдресСервера, "");

КонецФункции

// Возвращает порт сервера 1С
//
// Возвращаемое значение:
//   Число, Неопределено - Текущий порт сервера 1С, Неопределено в противном случае
//
Функция ПолучитьПортСервера() Экспорт

    Возврат ?(ЗначениеЗаполнено(ПортСервера), ПортСервера, Неопределено);

КонецФункции

// Возвращает состояние подключения к кластеру 1С
//
// Возвращаемое значение:
//   Булево - Подключен ли кластер 1С
//
Функция КластерПодключен() Экспорт

	Возврат ?(ЗначениеЗаполнено(КластерПодключен), КластерПодключен, Ложь);
...
Функция СписокИнформационныхБаз(Знач ИдентификаторКластера = Неопределено) Экспорт
	...
	Для каждого КлючЗначение Из ИндексИнформационныхБаз Цикл
		...
		НайденнаяСтрока = ТаблицаИнформационныхБазы.Найти(ОписаниеИБ.Имя, "Имя");
...
Процедура ОтключитьСоединенияИнформационнойБазы(Знач ИнформационнаяБаза, Знач Фильтр = Неопределено) Экспорт
	...
	Для Каждого Соединение Из СоединенияБазы Цикл
		...
		Лог.Отладка("Отключаю соединение %3 - <%1>, <%2>", 
					Соединение.Идентификатор,
...
//
Процедура ОтключитьСоединение(Знач ИдентификаторСоединения, 
				Знач ИдентификаторПроцесса, 
				Знач ИдентификаторКластер, 
				Знач ИнформационнаяБаза = Неопределено) Экспорт
...
Процедура СнятьБлокировкуИнформационнойБазы(Знач ИнформационнаяБаза, 
											...
											Знач ОчищатьПараметрыБлокировки = Истина) Экспорт
	...
	Если ОчищатьПараметрыБлокировки Тогда
...
//   ВыдачаЛицензийСервером - Булево - управление выдачей лицензий сервером 1С:Предприятия
//
//  Возвращаемое значение:
...
Функция СоздатьИнформационнуюБазу(Знач Кластер,
									...
									Знач ВыдачаЛицензийСервером = Ложь
									) Экспорт
	...
	Параметры.Добавить("--license-distribution=" + ?(ВыдачаЛицензийСервером, "allow", "deny"));
